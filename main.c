...
threadpool_t *threadpool_create(int min_thr_num, int max_thr_num, int queue_max_size) {
    ...
    do {
        ...
        pool -> busy_thr_num = 0; // 忙状态的线程初始值为0
        pool -> live_thr_num = min_thr_num; //活着的线程数 初值=最小线程数
        pool -> wait_exit_thr_num = 0; // 要销毁的线程个数初始值也为0
        ...
        /* 初始化互斥琐、条件变量 */
        // lock 锁本体 thread_counter 锁忙的线程个数
        ...
        pthread_attr_t attr; // 通过线程属性来设置游离态（分离态），creat第二个参数一般为null
        pthread_create(&(pool->adjust_tid), &attr, adjust_thread, (void *)pool); /* 启动管理者线程 */
...
void *threadpool_thread(void *threadpool) {
    ...
    threadpool_task_t  task; // 函数调用，任务
    while (true) {
        ...
        while( (pool->queue_size == 0) && (!pool->shutdown) ) //线程池没有任务且不关闭线程池。
        {
            ...
            // 都必须和一个互斥锁配合，以防止多个线程同时请求pthread_cond_wait()
        ...
        pool -> queue_front = ((pool -> queue_front + 1) % pool -> queue_max_size); // front后移
        pool -> queue_size--;
...
void *adjust_thread(void* threadpool) {
    ...
    while(!(pool->shutdown)) {
        ...
        if (queue_size >= MIN_WAIT_TASK_NUM && live_thr_num < pool -> max_thr_num) {
            ...
            for (i = 0; i < pool->max_thr_num && add < DEFAULT_THREAD_VARY && pool ->live_thr_num < pool -> max_thr_num; ++i) {
                if (pool -> threads[i] == 0 || !is_thread_alive(pool -> threads[i])) {
                    // void *(*start_rtn)(void *), 参数三，新创建的线程从start_rtn函数的地址开始运行
                    pthread_create(&(pool->threads[i]), NULL, threadpool_thread, (void*) pool);
                    add++;
                    pool -> live_thr_num++;
                }
        ...
        /* 销毁多余的空闲线程 算法：忙线程X2 小于存活的线程数 且存活的线程数 大于最小线程数时*/
